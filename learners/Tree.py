...
class linear_model:
    ...
    def to_string(self, features, logspace, d):
        ...
        if not logspace:
            ...
            lst = ["{}**{}".format(features[c], round_d(coefs[c])) for c in range(len(coefs)) if round_d(coefs[c]) != 0]
        ...
        if self.fit_intercept:
            if logspace:
                string += "*{}".format(
                    str(round_d(eval(f"exp_int_no_nan({self.model.intercept_})")))
                )
            else:
                string += "+{}".format(str(round_d(self.model.intercept_)))
        return string

    def to_string_C(self, features, logspace, d):
        coefs = self.model.coef_
        ...
        if not logspace:
            ...
            lst = ["(pow({}, {}))".format(features[c], round_d(coefs[c])) for c in range(len(coefs)) if round_d(coefs[c]) != 0]
...
def _split_traverse_node(
    node, model, splittable, max_depth, min_samples_leaf, logspace
):
    # Extract data
    X, y = node["data"]
    N, _ = X.shape
            ...
            for threshold in threshold_search:
                for sign in ["==", "<="]:
                    ...
                    loss_right, model_right = _fit_model(
                        X_right, y_right, model, logspace
                    )
...
def _build_tree(
    X_init, y_post, logspace, model, splittable, max_depth, min_samples_leaf
):
    """
...
def extract_invariant(node, features, logspace, depth, round_digit):
    ...
    if depth == 0 or (
        ...
        l = extract_invariant(
            node["children"]["left"], features, logspace, depth - 1, round_digit
        )
        r = extract_invariant(
            node["children"]["right"], features, logspace, depth - 1, round_digit
        )
        opsign = _op_sign(node["sign"])
        return "[{}]*({})+[{}]*({})".format(
            threshold_str, l, threshold_str.replace(node["sign"], opsign), r
        )


def extract_invariant_C(node, features, logspace, depth, round_digit):
    # Empty node
    if depth == 0 or (
        node["children"]["left"] is None and node["children"]["right"] is None
    ):
        return node["model"].to_string_C(features, logspace, round_digit)
    else:
        if logspace:
            print("WORKING IN LOGSPACE")
            threshold = exp_int_no_nan(node["threshold"])
        ...
        threshold_str = "{}({}, {:.3f})".format(
            sign_to_fcall(node["sign"]), features[node["j_feature"]], threshold
        )
        opsign = _op_sign(node["sign"])

        threshold_str_op = "{}({}, {:.3f})".format(
            sign_to_fcall(node["sign"]), features[node["j_feature"]], threshold
        )

        l = extract_invariant_C(
            node["children"]["left"], features, logspace, depth - 1, round_digit
        )
        r = extract_invariant_C(
            node["children"]["right"], features, logspace, depth - 1, round_digit
        )
        return "({})*({})+({})*({})".format(
            threshold_str, l, threshold_str_op, r
        )
...
def extract_loss(node, depth):
    if depth == 0 or (
        ...
        return (left_loss * lchild["n_samples"] + right_loss * rchild["n_samples"]) / (
            lchild["n_samples"] + rchild["n_samples"]
        )
...
def tree_depth(node):
    ...
    if no_children:
        ...
        return 1 + max(
            tree_depth(node["children"]["left"]), tree_depth(node["children"]["right"])
        )
        ...
        for depth in range(min(self.max_depth, tree_depth(tree) + 1)):
            ...
            if loss < var / 5:
                for round_digit in digits:
                    inv = extract_invariant(
                        tree, self.features, self.logspace, depth, round_digit
                    )

                    print(f"INV : {inv}")

                    c_inv = extract_invariant_C(
                        tree, self.features, self.logspace, depth, round_digit
                    )
                    print(
                        f"for depth {depth} and round {round_digit} the invaraint is:\n {inv}"
                    )

                    print("C version:{}".format(c_inv))
                    invlist.append((inv, loss * (1 + depth) * (1 + round_digit)))
                    invlist_c.append((c_inv, loss * (1 + depth) * (1 + round_digit)))

                    # scale loss by the complexity of the invariant
                
        # print(
        #     "From model trees learned, we extract the following candidate
        #     (sub)invariants: "
        # )
        # for inv in invlist:
        #     print(inv)
        return invlist, invlist_c
