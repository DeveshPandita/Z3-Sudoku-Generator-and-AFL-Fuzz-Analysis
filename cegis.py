...
def cegis_one_prog(
    ...
    while learning_time + verifying_time <= timeout:
        ...
        learners = get_learners(
            features_log, features_linear, log_not_split, linear_non_split
        )
        before_learning = time.time()
        cand_dic, cand_dic_c = learn_cand_dic(learners, data)
        for cand in cand_dic:
            print(cand)
        ...
        t = threading.Thread(
            target=verify_cand,
            args=(cand_dic, cand_dic_c, task, session, var_types, assumed_shape, results, config),
        )
        t.start()
        ...
        if verified:
            ...
            data = combine_data(data, add_data, more_sample)
    # Messages when timeout
    s = "We are not able to verify an invariant. \n We have learned the \
        following candidates for invariant: "
...
def learn_cand_dic(learners, data):
    ...
    for learner in learners:
        invlist, invlist_c = learner.learn_inv(data)
        idx = 0
        for inv in invlist:
            cand_dic[inv[0]] = inv[1]
            cand_dic_c[inv[0]] = invlist_c[idx][0]
            idx += 1

    return cand_dic, cand_dic_c
...
def verify_cand(cand_dic, cand_dic_c, task, session, var_types, assumed_shape, results, json_config):
    counter_examples = []
    sorted_keys = sorted(cand_dic, key=cand_dic.get, reverse=False)[:5]
    for inv in sorted_keys:
        print("Trying to verify {}".format(inv))
        print("Trying to verify C expression {}".format(cand_dic_c[inv]))
        inv_verifier = Verifier(inv, cand_dic_c[inv], assumed_shape, task, session, json_config)
