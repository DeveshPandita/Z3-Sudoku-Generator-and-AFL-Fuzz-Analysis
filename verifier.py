...
class Verifier:
    ...
    def compute_conditions_exact(self, var_types):
        ...
        # codegen.print_green("Compiling C code...")
        # compiled = cgen.compile_with_afl_clang_fast()
        # if compiled:
        #     codegen.print_green("Compilation successful.")

        # fuzzstat = cgen.fuzz_check('target.exe', 'indir', 'outdir')
        # if fuzzstat == "timeout":
        #     # codegen.print_red("Timeout occurred. Exiting...")
        #     return []
        
        # TODO: read the crash file and parse the crash

        var_types = ast.literal_eval(self.config['cgen']['vars'])
        is_valid, cex_list = cgen.validate_invariant(self.invexpr_c, self.task, var_types)
        if is_valid:
            return []

        codegen.print_red("Invariant is not valid.")
    
        # max element in cex_list with respect to self.json_config['cgen']['probs']
        # pb = self.config['cgen']['probs'][0]
        # max_cex = max(cex_list, key=lambda x: x[pb])

        # max_len = min(len(cex_list), 3)
        # print(f"CEX list: {cex_list[-max_len:]}")

        # return cex_list[-max_len:]
        return cex_list
        ...
        # inv_pre = compute_lhs_pre(lhs, self.task["assignments"])
        # pre = self.task["pre"][0]
        # # pre <= I
        # maxpre, inspre = maximize(evaluate(pre, inv_pre, "-"), var_types)
       
        # rhs = wp_expression(whole_inv, self.task["loopbody"])
        # # [G]*([G] ·[inv'] + post) =  wpe(P, [G] · [inv'] + post)
        # maxlmr, inslgr = maximize(evaluate(lhs, rhs, "-"), var_types)
        # # maxrml, insrgl = maximize(evaluate(rhs, lhs, "-"), var_types)
       
        # final = maxlmr + inslgr + maxpre + inspre
        ...
        # print(
        #     "          Wolfram alpha finds {} counterexamples\
        #     ".format(
        #         len(output_list)
        #     )
        # )
        ...
        #     try:
        #         violation = False
        #         for lh, rh in [(pre, inv_pre), (lhs, rhs)]:
        #             lh = (
        #                 lh.replace("[", "(")
        #                 .replace("]", ")")
        #                 .replace("^", "**")
        ...
        #                 .replace("&", ")*(")
        #             )
        #             lhs_val = eval(lh, {"__builtins__": None}, cex)
        #             rhs_val = eval(rh, {"__builtins__": None}, cex)
        ...
        # print(
        #     "{} of those counterexamples are indeed counterexamples:".format(len(res))
        # )
        # for cex in res:
        #     print("{}".format(cex))
...
def wp_expression(inv, loopbody):
    ...
    for ele in loopbody:
        ...
        if len(parts) == 1:
            ...
            if is_equalities(
                parts[1]
            ):  # this is the case for handling conditionals `if then else`
...
def maximize(out: str, var_types):
    ...
    out = out.split("||")  # type: ignore
    out = [i.replace("or", "||") for i in out]  # type: ignore
    out = [i.replace("not", "!") for i in out]  # type: ignore
    out = [i[1:-1] for i in out]  # type: ignore
    out = [
...
def extract_wl_output(session, output, ib_list):
    print("OUTPUT FROM EXTRACT WL OUTPUT: {}".format(output))
    temp_output = []
...
def compute_lhs_pre(inv, assignments):
    for ele in assignments:
        var, val = ele.split("==")[0].strip(), ele.split("==")[1].strip()
        inv = inv.replace(var, val)
    return inv
